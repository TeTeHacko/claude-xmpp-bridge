...
When relay/broadcast traffic is injected back into an agent terminal, the bridge wraps it as a clearly generated block:

```text
[bridge-generated message]
{"type":"relay","generated":true,"from":"ses_A","to":"ses_B","mode":"screen","message_id":"abc123def456","ts":1741612800.123}

hello from another agent
```

This makes it obvious in shared GNU Screen/tmux windows that the text was generated by the bridge, not typed by a human.
...
| `cleanup_stale_locks(project="")` | Remove stale bridge-native and legacy file locks whose `session_id` is no longer active |
...
- Debounces Screen title updates and avoids title redraws on every tool call, reducing TUI artefacts in GNU Screen
...
- Polls MCP inbox on `session.idle` and every 30 s — injects pending inter-agent messages into the session, reusing the current MCP `mcp-session-id` between polls to avoid redundant `initialize` requests
...
- Logs bridge/plugin warnings, errors, and caught handler exceptions via OpenCode's structured plugin log instead of dumping raw diagnostics into the terminal, captures bridge CLI/helper stderr internally so expected client or helper failures do not print into the OpenCode TUI, and skips optional helper commands entirely when they are unavailable
