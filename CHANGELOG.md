...
### Fixed
- **Dual injection strategy** — try SDK `promptAsync` first, fall back to TUI `appendPrompt` + `submitPrompt` if unavailable or failing. TUI approach simulates user input and works reliably in all modes.

## [0.9.5] - 2026-03-22

### Fixed
- **Add SDK method availability check and detailed logging** — check if `client.session.promptAsync` exists before calling, log SDK response details (status, error, data) for debugging.
...
### Fixed
- **Fix prompt_async URL resolution** — `input.serverUrl` is a URL object getter, not a string. Read it lazily at call time (not at plugin init) and convert via `.href`. Fallback to `OPENCODE_SERVER_URL` env or `http://localhost:4096`.
...
### Added
- **OpenCode plugin: CML agent icon** — added 🧠 (brain) icon for the `cml` agent (Centrální Mozek Lidstva orchestrator) to the agent icon mapping, header comments, and env variable documentation.
...
### Added
- **OpenCode plugin detects question parts in assistant messages** — when the agent asks a question (interactive question tool), the screen window title switches to 🔴 (interaction) instead of 🟢 (idle), making it immediately visible that the agent is waiting for user input. The bridge state is also reported as `interaction`.
